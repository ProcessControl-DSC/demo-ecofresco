 ...
 * shows a small "work screen" confirmation with a live chronometer (HH:MM:SS)
 * plus two big buttons — "Pausa" and "Fichar salida" — so the operator can
 * close the workday (or take a break) without leaving this app: they don't
 * need to walk back to the main kiosk screen and re-enter their PIN. Both
 * buttons reuse the native check-in/check-out toggle (hr_attendance has no
 * separate "break" state — a pause IS a check-out; resuming later is a
 * normal check-in again).
 *
 * The chronometer ticks locally in the browser from a base value supplied by
 * the server (elapsed_seconds computed from hr.attendance.check_in / the
 * final worked_hours), avoiding any client-side timezone/date parsing:
 * - Attendance still open  → ticks up every second (live work time).
 * - Attendance already closed → stays static (final worked time).
 *
 * Props:
...
export class PcMesTargetSelector extends Component {
    ...
    setup() {
        this.state = useState({
            ...
            confirmedTaskName: "",
            quickEndError: null,
            // Chronometer (see class docstring)
            attendanceIsOpen: false,
            elapsedSeconds: 0,
        });
        this._loadTargets();

        // Tick the chronometer every second. Only advances the displayed
        // value while the attendance is still open; harmless no-op once
        // closed (elapsedSeconds is then a static final value).
        this._chronoInterval = setInterval(() => {
            if (this.state.attendanceIsOpen) {
                this.state.elapsedSeconds += 1;
            }
        }, 1000);
        onWillDestroy(() => clearInterval(this._chronoInterval));
    }
    ...
    async _loadTargets() {
        try {
            ...
            this._applyAttendanceChrono(result.attendance);
        } catch (_err) {
    ...
    /** Apply the {is_open, elapsed_seconds} chronometer payload from the
     * server onto local state. No-op if the payload is missing/null. */
    _applyAttendanceChrono(attendance) {
        if (!attendance) {
            return;
        }
        this.state.attendanceIsOpen = !!attendance.is_open;
        this.state.elapsedSeconds = attendance.elapsed_seconds || 0;
    }

    /** Chronometer formatted as HH:MM:SS for display on the work screen. */
    get chronoDisplay() {
        const total = Math.max(0, Math.floor(this.state.elapsedSeconds));
    ...
    async confirmSelection(projectId, taskId) {
        ...
        try {
            const result = await rpc("/pc_mes_kiosk/set_target", {
                employee_id: this.props.employeeId,
                project_id: projectId || null,
                task_id: taskId || null,
            });
            this._applyAttendanceChrono(result && result.attendance);
     ...
     * the operator now sees the chronometer plus the Pausa / Fichar salida
     * controls, plus a Continuar button to head back to the greet screen.
     */
    ...
    /** Skip target selection, but still show the work screen (chronometer and
     * Pausa / Fichar salida must stay reachable even if no project/task was
     * set). The chronometer already reflects the state fetched at mount. */
    skip() {
